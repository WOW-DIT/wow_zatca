...
import base64
from .csr import CSR
from .report_invoice import report_invoice, clear_invoice, check_compliance
...
@frappe.whitelist()
def check_invoice_compliance(invoice_name):
    doc = frappe.get_doc("Sales Invoice", invoice_name)
    company = frappe.get_doc('Company', doc.company)
    uuid_value = str(uuid.uuid4())
    
    invoice_items = doc.items
    if company.zatca_v2 == 0:
        return
    ...
    previous_invoices = frappe.db.sql("""
        ...
        WHERE i.company = %s AND i.status = %s AND i.egsunit = %s
        ORDER BY i.invoice_counter DESC
        LIMIT 1;
    """, (company.name, "Paid", egsunit.name), as_dict=True)
